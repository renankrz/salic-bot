...
salic-bot \
  ...
  --pronac 123456 \
  --despesas-csv /caminho/para/despesas.csv \
...
Todos os argumentos são opcionais na linha de comando — se omitidos, o bot busca o valor em outras fontes (veja a seção de precedência abaixo). Porém, `proponente`, `pronac`, `despesas-csv`, `comprovantes-dir`, `cpf` e `senha` são obrigatórios e precisam estar definidos em pelo menos uma fonte.
...
| Campo | Precedência |
|---|---|
| mecanismo, proponente, pronac, despesas_csv, comprovantes_dir | argumento CLI → `.env` → QSettings → default |
...
# CLI
uv run salic-bot --proponente 11222333000181 --pronac 123456 --despesas-csv /caminho/para/despesas.csv --comprovantes-dir /caminho/para/comprovantes --cpf 11122233344 --senha SuaSenha
```
